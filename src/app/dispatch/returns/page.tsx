...
import { DispositionDialog } from '@/app/warehouse/returns/_components/DispositionDialog'
import {
  MOCK_RETURNS,
  STATUS_BADGE,
  DISPOSITION_LABEL,
...
export default function DispatchReturnsPage() {
  ...
  function handleReceive(orderId: string) {
    setReturns(prev => prev.map(r =>
      r.id === orderId ? { ...r, status: 'arrived', receivedBy: 'Chika Obi', receivedAt: new Date().toISOString() } : r
    ))
  }
  ...
  return (
    <AppShell
      role="dsp_sup"
      currentPath="/dispatch/returns"
      title="Returned Items"
      breadcrumb={[{ label: 'Dashboard', href: '/dispatch' }, { label: 'Returns' }]}
    >
