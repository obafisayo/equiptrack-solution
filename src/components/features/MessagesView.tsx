...
import { ChatView } from '@/app/messages/_components/ChatView'
import { ContactProfile } from '@/app/messages/_components/ContactProfile'
import { MOCK_CONVERSATIONS, type Conversation, type ChatMessage } from '@/app/messages/_components/types'
  ...
  return (
    ...
    */
    ...
    >
      ...
      <div className={[
        'w-full md:w-70 md:shrink-0 border-r border-border-default flex flex-col',
      ...
      {/* Column 3: Profile panel — desktop only */}
      {selected && (
        <>
          {!showProfile && (
            <button
          ...
          {showProfile && (
            <div className="hidden lg:flex w-64 shrink-0 border-l border-border-default overflow-y-auto flex-col">
              <ContactProfile conversation={selected} onClose={() => setShowProfile(false)} />
            </div>
          )}
        </>
